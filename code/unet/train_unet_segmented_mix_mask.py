...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for imgs, targets, valid_fire, valid_smoke in train_loader:
            ...
            loss_map = criterion(logits, targets)     # (B, 2, H, W)

            # Fire loss (channel 0)
            fire_denom = valid_fire.sum()
            if fire_denom > 0:
                fire_loss = (loss_map[:, 0] * valid_fire).sum() / fire_denom
            else:
                fire_loss = 0.0

            # Smoke loss (channel 1)
            smoke_denom = valid_smoke.sum()
            if smoke_denom > 0:
                smoke_loss = (loss_map[:, 1] * valid_smoke).sum() / smoke_denom
            else:
                smoke_loss = 0.0
